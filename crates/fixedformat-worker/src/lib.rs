//! The `fixedformat` VGI worker (library).
//!
...
//! the `fixedformat-wasm` crate (the browser build, which serves the same
//! `Worker` over a SharedArrayBuffer byte channel instead).
//!
...
pub mod arrow_map;
mod buffering;
// Public so the wasm entrypoint crate can run its transport self-test against
...
mod meta;
mod options;
// Public for the benchmark harness (`bench_scan_local`) and the wasm crate's
// transport self-test; not part of the SQL surface.
pub mod reader;
mod record_writer;
mod scalar;
mod table;
mod value_in;
// Browser transport + crypto for object_store. Native builds get both from
// object_store itself, so this module only exists on wasm.
...
/// The catalog name DuckDB sees in `ATTACH 'fixed' (TYPE vgi, …)`. Defaults to
/// `fixed`, but honors an explicit override so a test harness can rename it.
...
/// Build a fully-registered worker: every scalar, table, buffering, and COPY
...
/// Benchmark harness: scan files or URLs through the **production** read pipeline
/// and report `(rows, batches)`.
///
/// Mirrors `read_fixed`'s `StreamingProducer`: framing, decompression,
/// `decode_record`, and Arrow construction, one `batch_rows` batch at a time
/// with the batch dropped after it is built — so peak memory is ~one batch, as
/// in a real scan. (Do NOT use `read_all` here: it accumulates every row, which
/// makes a benchmark allocation-bound in a way production is not.)
/// Available on every target so native and browser numbers are comparable.
///
/// Not part of the SQL surface; exposed for load testing only.
#[doc(hidden)]
pub fn bench_scan_local(
    paths: &[String],
    spec: &str,
    batch_rows: usize,
) -> Result<(usize, usize), String> {
    bench_scan_opts(paths, spec, batch_rows, &[])
}

/// As [`bench_scan_local`], with object-store `overrides` (the same
/// `aws_*` key/value pairs `cloud::build_store` accepts) so a benchmark can hit
/// a credentialed `s3://` endpoint.
#[doc(hidden)]
pub fn bench_scan_opts(
    paths: &[String],
    spec: &str,
    batch_rows: usize,
    ...
    use fixedformat_core::layout::parse_spec;
    use fixedformat_core::stream::Limits;
    use fixedformat_core::Encoding;

    let layout = parse_spec(spec, None).map_err(|e| e.to_string())?;
    let schema = std::sync::Arc::new(arrow_schema::Schema::new(
        arrow_map::layout_fields(&layout).map_err(|e| e.to_string())?,
    ));
    let projection: Vec<usize> = (0..schema.fields().len()).collect();
    // Classify so the same harness covers local/OPFS paths and http(s):// URLs.
    let locations: Vec<cloud::Location> = paths
        .iter()
        .map(|p| cloud::classify(p).map_err(|e| e.to_string()))
        .collect::<Result<_, _>>()?;
    let sources = reader::resolve_sources(&locations, &vgi::secrets::Secrets::default(), overrides)
        .map_err(|e| e.to_string())?;

    let batch_rows = batch_rows.max(1);
    let mut total = 0usize;
    let mut batches = 0usize;
    let mut pending: Vec<Vec<fixedformat_core::Value>> = Vec::with_capacity(batch_rows);

    for source in &sources {
        let label = source.label().to_string();
        let stream = reader::open_stream(source, Framing::Newline, 0, None, Limits::default())
            .map_err(|e| e.to_string())?;
        for (i, rec) in stream.enumerate() {
            let rec = rec.map_err(|e| format!("{label}: {e}"))?;
            let pairs = decode_record(&layout, &rec, Encoding::Ascii)
                .map_err(|e| format!("{label}: record {}: {e}", i + 1))?;
            pending.push(pairs.into_iter().map(|(_, v)| v).collect());
            total += 1;
            if pending.len() == batch_rows {
                let b = reader::build_batch(&schema, &projection, std::mem::take(&mut pending))
                    .map_err(|e| e.to_string())?;
    ...
    if !pending.is_empty() {
        let b = reader::build_batch(&schema, &projection, pending).map_err(|e| e.to_string())?;
        std::hint::black_box(&b);
        batches += 1;
    }
    Ok((total, batches))
