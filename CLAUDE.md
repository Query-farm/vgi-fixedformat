...
- `crates/fixedformat-core` — pure codecs, **no Arrow/VGI deps** (`unsafe`
  ...
  (PICTURE-editing print-image numerics) / `ebcdic` (CP037 tables) / `framing`
  (slice splitter) / `stream` (streaming framer + `decompress_reader`) /
  `compression` (gzip/zstd). All correctness lives here, unit-tested directly.
